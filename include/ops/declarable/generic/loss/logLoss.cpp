...
CUSTOM_OP_IMPL(log_loss, 3, 1, false, 1, 1) {
    ...
    // input validation
    REQUIRE_TRUE(labels->isSameShape(predictions), 0, "CUSTOM_OP loss function log_loss: labels and predictions arrays have different shapes!");
    // weights array can be single scalar or has the same rank as labels, and must be broadcastable to labels
    REQUIRE_TRUE(!(!weights->isScalar() && weights->rankOf() != labels->rankOf()), 0, "CUSTOM_OP loss function log_loss: weights array must have the same rank as labels array!");
    // check whether broadcast operation is possible for weights array
    if(!weights->isScalar())
    	for (int i = 0; i < weights->rankOf(); ++i)
        	REQUIRE_TRUE(!(weights->shapeOf()[i] != labels->shapeOf()[i] && weights->shapeOf()[i] != 1), 0, "CUSTOM_OP loss function log_loss: shapes of weights array is not broadcastable to labels shape!");
    ...
    REQUIRE_TRUE(reductionMode==0 || reductionMode==1 || reductionMode==2 || reductionMode==3, 0, "CUSTOM_OP loss function log_loss: reduction mode has not acceptable value, possible values are 0, 1, 2, 3 !");
