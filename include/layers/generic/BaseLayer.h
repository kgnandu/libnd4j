...
template<typename T, typename AF> void BaseLayer<T,AF>::dropOutHelper(NDArray<T> *input) {
    ...
    NativeOpExcutioner<T>::execRandom(2, (Nd4jPointer) this->_rng, input->getBuff(), input->getShapeInfo(), input->getBuff(), input->getShapeInfo(), extras);
...
template<typename T, typename AF> void BaseLayer<T,AF>::dropConnectHelper(NDArray<T> *input) {
    ...
    NativeOpExcutioner<T>::execRandom(1, (Nd4jPointer) this->_rng, input->getBuff(), input->getShapeInfo(), input->getBuff(), input->getShapeInfo(), extras);
